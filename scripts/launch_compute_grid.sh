...
usage() {
    cat <<'EOF'
Usage: launch_compute_grid.sh [--gpu ID] [--sample-every SECONDS] [--datasets CSV] [--dry-run]

Runs the histogram compute benchmark grid sequentially with profiling.
EOF
}
...
dataset_k_order() {
    case "$1" in
        ehrshot) echo "64 512 128 256" ;;
        mimic) echo "2048 16384 4096 8192" ;;
        *)
            echo "Unknown dataset: $1" >&2
            exit 1
            ;;
...
train_batch_size() {
    ...
    case "${dataset}:${resolution}" in
        ...
        mimic:month|mimic:quarter_annual) echo "256" ;;
        mimic:semi_annual) echo "256" ;;
        *)
...
inference_batch_size() {
    ...
    case "${dataset}:${resolution}" in
        ehrshot:day|ehrshot:week|ehrshot:quarter_annual|ehrshot:semi_annual) echo "512" ;;
        ehrshot:month) echo "512" ;;
        mimic:day|mimic:week|mimic:quarter_annual|mimic:semi_annual) echo "512" ;;
        mimic:month) echo "512" ;;
        *)
            echo "Unknown dataset/resolution pair: ${dataset}/${resolution}" >&2
            exit 1
            ;;
...
run_profile() {
    ...
    local run_name="${dataset}_K${k}_${resolution}_${stage}"
    local summary_path="${metrics_dir}/${run_name}_summary.json"

    mkdir -p "${metrics_dir}"

    if [[ -f "${summary_path}" ]]; then
        log "Skipping ${run_name}; summary already exists at ${summary_path}"
        return
    fi
...
run_one_combo() {
    ...
    local bench_root="${base_dir}/eic/compute_benchmark/histogram_grid"
    ...
    local fit_gap
    fit_gap="$(resolution_fit_gap "${resolution}")"
    local train_gap
    train_gap="$(resolution_train_gap "${resolution}")"
    local window_days
    ...
    local quantizer_output_dir="${bench_root}/results/em_quantizer/${folder_name}/"
    local ar_output_dir="${bench_root}/results/em_autoregressive/${folder_name}/"
    local trajectories_output_dir="${bench_root}/results/em_generated_trajectories/${folder_name}/"
    local fit_cache_dir="${bench_root}/fit_em_cache/${resolution}_level/"
    local profiling_dir="${bench_root}/profiling/${dataset}/${resolution}/"
    ...
    run_profile \
        ...
        "gap_config.max_gap_length=${fit_gap}" \
    ...
    run_profile \
        ...
        "gap_config.max_gap_length=${train_gap}" \
        "window_size_days=${window_days}" \
        "quantizer_config.n_embeddings=${k}" \
...
log "Histogram compute grid complete"
