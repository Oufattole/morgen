...
class MorgenModule(L.LightningModule):
    """A LightningModule for training and evaluating the morgen model."""

    @classmethod
    def load_from_checkpoint(cls, ckpt_path: Path | None = None) -> "MorgenModule":
        ...
        Returns:
            The loaded MorgenModule instance, with all hyperparameters matching _except_ for the optimizer
        ...
        Examples:
            ...
            >>> module = MorgenModule(model=model, metrics=metrics, optimizer=None)
        ...
        Now, we can save the checkpoint to a temporary file and load it back in:
            ...
            ...     loaded_module = MorgenModule.load_from_checkpoint(f.name)
        ...
        inference_mode = hparams.get("inference_mode", "generate")
    ...
    def _is_norm_bias_param(n: str) -> bool:
        ...
        Examples:
            >>> MorgenModule._is_norm_bias_param("model.decoder.bias")
            True
            >>> MorgenModule._is_norm_bias_param("model.layernorm.weight")
            True
            >>> MorgenModule._is_norm_bias_param("model.LayerNorm12.weight")
            True
            >>> MorgenModule._is_norm_bias_param("model.decoder.weight")
            False
            >>> MorgenModule._is_norm_bias_param("model.HF_model.gpt_neox.final_layer_norm.weight")
            True
        """
    ...
    def weight_decay(self) -> float | None:
        ...
        Examples:
            ...
            ... })
            >>> metrics = NextCodeMetrics(top_k=[1, 2, 3], vocab_size=4)
            >>> MorgenModule(model=model, metrics=metrics, optimizer=opt_factory).weight_decay
            0.01
        """
    ...
    def optimizer_no_decay_factory(
        ...
        Examples:
            ...
            ... })
            >>> metrics = NextCodeMetrics(top_k=[1, 2, 3], vocab_size=4)
            >>> module = MorgenModule(model=model, metrics=metrics, optimizer=opt_factory)
    ...
    def predict_step(self, batch: MEDSTorchBatch):
        """Generate trajectories for a batch."""
