...
class Model(torch.nn.Module):
    ...
    We can also run the model on a sample batch of data. This `sample_batch` is a MEDSTorchBatch object.
    The only feature of this batch that we use in this model is the `code` key.
    ...
    def forward_hidden_states(
        ...
        *,
        ...
        """Run a forward pass that returns last-layer hidden states.

        Args:
            batch: Input batch containing ``code`` and padding metadata.
            detach: Whether to detach the returned hidden states from the computation graph.
            use_cache: Forward to the underlying HF model for completeness (defaults to ``False`` so the
                returned hidden states cover the entire sequence).
            output_attentions: Whether to ask the HF model for attention weights.

        Returns:
            A tuple ``(hidden_states, outputs)`` where ``hidden_states`` is the last-layer hidden representation
            with shape ``[batch, seq_len, hidden_size]`` and ``outputs`` is the full
            :class:`~transformers.CausalLMOutputWithPast` object from Hugging Face.
        """

        hf_inputs = self._hf_inputs(batch)
